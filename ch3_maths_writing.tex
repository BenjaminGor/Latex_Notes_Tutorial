...
Three other types of symbols that may be of interest come from \texttt{\textbackslash mathbb\{<character>\}}\index{mathbb@\texttt{\textbackslash mathbb}}, \texttt{\textbackslash mathcal\{<character>\}}\index{mathcal@\texttt{\textbackslash mathcal}}, and \texttt{\textbackslash mathscr\{<character>\}}\index{mathscr@\texttt{\textbackslash mathscr}} (the last one requires the package \texttt{mathrsfs}\index{mathrsfs@\texttt{mathrsfs}}) for sets, classes, or operators. For example, the set of all real numbers is commonly denoted by $\mathbb{R}$ (\texttt{\textbackslash mathbb\{R\}}), while the class of continuously differentiable functions is denoted by $\mathcal{C}^1$ (\texttt{\textbackslash mathcal\{C\}\^{}1}). Laplace Transform is sometimes denoted by $\mathscr{L}$ (\texttt{\textbackslash mathscr\{L\}}). Moreover, in some situations we may want to type math characters in upright, and we can use \texttt{\textbackslash mathrm\{<character>\}}\index{mathrm@\texttt{\textbackslash mathrm}}, e.g.\ for a differential $\mathrm{d}x$ (\texttt{\textbackslash mathrm\{d\}x}).

\paragraph{Prescripts} To add a superscript or subscript but at the left instead of the default right, we can utilize the \texttt{\textbackslash prescript}\index{prescript@\texttt{\textbackslash prescript}} command. For example,
\begin{lstlisting}
$\prescript{12}{6}{\text{C}}$
\end{lstlisting}
is rendered as $\prescript{12}{6}{\text{C}}$.
