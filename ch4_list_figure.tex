...
\paragraph{Customizing Rules in a Table}
It is also common to change the color and width of rules in a table for aesthetic reasons. A small example is shown by Table \ref{tab:rudin} as follows.
\begin{lstlisting}
\begin{table}[ht!]
\setlength{\arrayrulewidth}{0.5ex}
\centering
\begin{tabularx}{\textwidth}{p{0.5\textwidth}cccc}
\arrayrulecolor{Red}
 & Open & Closed & Perfect & Bounded \\
\hline
The set of all complex $z$ such that $\abs{z} < 1$ & No & Yes & No & Yes \\
...
\end{tabularx}
...
\end{lstlisting}
...
\hline
The set of all complex $z$ such that $\abs{z} < 1$ & No & Yes & No & Yes \\
The set of all complex $z$ such that $\abs{z} \leq 1$ & Yes & No & Yes & Yes \\
The set of all integers & Yes & No & No & No \\
\end{tabularx}
\caption{Basic topological properties of some selected sets. (adapted from Baby Rudin)}
\label{tab:rudin}
\end{table}
The \texttt{\textbackslash setlength\{\textbackslash arrayrulewidth\}\{\}}\index{arrayrulewidth@\texttt{\textbackslash arrayrulewidth}} and \texttt{\textbackslash arrayrulecolor}\index{arrayrulecolor@\texttt{\textbackslash arrayrulecolor}} parts obviously indicate the new rule width and color, respectively. Bear in mind that the latter requires setting the option \texttt{table} when importing the \texttt{xcolor} package.
...
\end{minipage}    
\end{center}
The \verb|b| option indicates that the two blocks will be bottom-aligned (the same can be done for a \texttt{tabularx} object), provided that their width is fixed to $0.48$ times \texttt{\textbackslash textwidth} and thus they fit in the main text area.
