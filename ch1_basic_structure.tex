...
The \LaTeX{} system records and updates the \textit{numbering}\index{Numbering} for chapters/sections internally every time they are initialized. However, we can manually override that by invoking the command \texttt{\textbackslash setcounter\{chapter\}\{<number\allowbreak>\}}\index{setcounter@\texttt{\textbackslash setcounter}}. To achieve the effect of Chapter \ref{chap:install} at the very beginning, we use \texttt{\textbackslash setcounter\allowbreak\{chapter\}\{-1\}} so that the counter\index{Counter} increases by $1$ to $0$ as intended when Chapter \ref{chap:install} is generated. The same can be done for sections.
